...
def get_ydl_opts_youtube(cookies_path, client_list=None, po_token=None, visitor_data=None) -> dict:
    if client_list is None:
        client_list = _YT_CLIENT_ROTATION[0]

    opts = _base_opts()
    opts.update({
        # bestvideo*+bestaudio fetches full format list including merged ones
...
def process_youtube_formats(formats_raw: list, duration) -> list:
    """
    Only return formats that have BOTH video and audio (merged).
    YouTube merged formats exist up to 720p. 1080p+ are video-only on YouTube's
    servers — we skip them entirely so the user never sees a no-audio format.
    We also return the best audio-only stream separately.
    """
    formats = []
    seen: set[str] = set()
    ...
    # Only merged (has both video AND audio)
    merged = [
    ...
    # Sort merged by height desc, then fps desc
    merged.sort(key=lambda f: (f.get("height") or 0, f.get("fps") or 0), reverse=True)
    ...
    for fmt in merged:
        ...
        # Dedup by height + fps + hdr
        key = f"{height or 'best'}_{'60' if is_60fps else '30'}_{'hdr' if is_hdr else 'sdr'}"
        if key in seen:
            continue
        seen.add(key)
        ...
        formats.append({
            ...
            "has_audio": True,  # always True here — we only add merged formats
        })

    # If no merged formats found at all, log it
    if not formats:
        log.warning("No merged (audio+video) formats found for this YouTube video")
